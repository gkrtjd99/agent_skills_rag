...
## Task 16: Rewrite README.md (English)
...
A local RAG that searches skills under `~/.skills/` and loads only relevant
skill bodies into the agent context.

At session start only one meta-skill is auto-loaded. All other skills are
searched per user message through MCP, and only matching bodies are fetched.
...
- Vector DB: LanceDB
- Index: auto-syncs on `search_skills` with a 30s TTL
...
4. Prints MCP registration guidance

Register the MCP server in each harness and restart it.
...
---
name: my-skill
description: One-line description. Search quality depends on this.
---
...
The next `search_skills` call reflects the new file within 30 seconds.
...
| `uv run skill-rag eval` | Measure recall@5 |
...
| `SKILL_RAG_SCORE_THRESHOLD` | `0.35` | Match threshold |
| `SKILL_RAG_SYNC_TTL` | `30` | Sync cache TTL in seconds |

## Docs

- `AGENTS.md` — read order before an agent's first task
